...
# Editor (just a tad): Matt Preston
...
def create_bai(bam):
    header, err = Popen(["samtools","index",bam], stdout=PIPE, stderr=PIPE).communicate()
    if err != "":
        raise Exception(err)
...
def coverage(bam, mtchr = None):
    ...
    # Check to see if file has been indexed
    if os.path.isfile(bam + ".bai") == False:
        create_bai(bam)
    ...
    for c in contigs.keys():
        try:
            ...
            coverage_dict[c] = {}
            templist = Popen(command, stdout=PIPE, shell=True).communicate()[0].strip().split("\t")
            # Samtools returns nothing if contig not found in bam
            if templist == ['']:
                coverage_dict[c]["Bases Mapped"] = coverage_dict[c]["Sum of Depths"] = 0
            else:
                coverage_dict[c]["Bases Mapped"], coverage_dict[c]["Sum of Depths"] = map(int, templist)
            coverage_dict[c]["Length"] = int(contigs[c])
            # If length is zero (somehow, length can be non-zero even when gene isn't found)
            if coverage_dict[c]["Length"] == 0:
                coverage_dict[c]["Breadth of Coverage"] = coverage_dict[c]["Depth of Coverage"] = 0
            else:
                coverage_dict[c]["Breadth of Coverage"] = coverage_dict[c]["Bases Mapped"] * 100 / float(contigs[c])
                coverage_dict[c]["Depth of Coverage"] = coverage_dict[c]["Sum of Depths"] * 100 / float(contigs[c])                
        except:
            print "Creating gene coverage statistics failed"
            print "Error at gene: ", c
            raise
    ...
    try:
        ...
        if coverage_dict["genome"]["Length"] == 0:
            coverage_dict["genome"]["Breadth of Coverage"] = coverage_dict["genome"]["Depth of Coverage"] = 0
        else:
            coverage_dict["genome"]["Breadth of Coverage"] = sum([x["Bases Mapped"] for k, x in coverage_dict.iteritems() if k != "genome"]) * 100 / float(genome_length)
            coverage_dict["genome"]["Depth of Coverage"] = sum([x["Sum of Depths"] for k, x in coverage_dict.iteritems() if k != "genome"]) * 100 / float(genome_length)
    except:
        print "Creating genome coverage statistics failed"
        raise
...
#            if coverage_dict["nuclear"]["Length"] == 0:
#                coverage_dict["nuclear"]["Breadth of Coverage"] = coverage_dict["nuclear"]["Depth of Coverage"] = 0
#            else:
#                coverage_dict["nuclear"]["Breadth of Coverage"] = sum([x["Bases Mapped"] for k, x in coverage_dict.iteritems() if k not in ignore_contigs]) * 100 / float(coverage_dict["nuclear"]["Length"])
#                coverage_dict["nuclear"]["Depth of Coverage"] = sum([x["Sum of Depths"] for k, x in coverage_dict.iteritems() if k not in ignore_contigs]) * 100 / float(coverage_dict["nuclear"]["Length"])
...
#    except:
#        print "Creating nuclear coverage statistics failed"
#        raise
